...
class Window {
	...
	function onEvent(e:#if hldx dx.Event #else sdl.Event #end):Bool {
		...
		switch (e.type) {
			...
			case KeyDown:
				...
				if (e.keyCode & (1 << 30) != 0)
					e.keyCode = (e.keyCode & ((1 << 30) - 1)) + 1000;
			...
			case TextInput:
				...
				eh.charCode = if (c < 0x7F) c; else if (c < 0xE0) ((c & 0x3F) << 6) | ((e.keyCode >> 8) & 0x7F); else if (c < 0xF0)
					((c & 0x1F) << 12) | (((e.keyCode >> 8) & 0x7F) << 6) | ((e.keyCode >> 16) & 0x7F); else
	...
	public static function getMonitors():Array<Monitor> {
		return [
			for (m in #if hldx dx.Window.getMonitors() #elseif hlsdl sdl.Sdl.getDisplays() #else [] #end)
				{
					name: m.name,
